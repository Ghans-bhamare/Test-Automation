...
using BrowserStackSDK.TestObservability.Helper.Logger;
...
internal static class Initializer
{
    ...
    internal static void Run() {
        ...
        string[] attributes = { "Microsoft.VisualStudio.TestTools.UnitTesting.TestMethodAttribute", "Microsoft.VisualStudio.TestTools.UnitTesting.DataTestMethodAttribute" };
...
class GoToUrlPatch
{
    ...
    static Exception Finalizer(Exception __exception)
    {
        ...
        if (driver != null)
        {
                            ...
                            var sessionName = Regex.Replace(
                                (new StackTrace()).GetFrame(1).GetMethod().ToString().Split(" ")[1].Split("(")[0],
                                @"(_Patch)(\d+)(?!.*\d)",
                                ""
                            );
                            var status = "passed";
            ...
            if (message != null && (message.Contains("ERR_FAILED") || message.Contains("ERR_TIMED_OUT") || message.Contains("ERR_BLOCKED_BY_CLIENT") || message.Contains("ERR_NETWORK_CHANGED") || message.Contains("ERR_SOCKET_NOT_CONNECTED") || message.Contains("ERR_CONNECTION_CLOSED") || message.Contains("ERR_CONNECTION_RESET") || message.Contains("ERR_CONNECTION_REFUSED") || message.Contains("ERR_CONNECTION_ABORTED") || message.Contains("ERR_CONNECTION_FAILED") || message.Contains("ERR_NAME_NOT_RESOLVED") || message.Contains("ERR_ADDRESS_INVALID") || message.Contains("ERR_ADDRESS_UNREACHABLE") || message.Contains("ERR_TUNNEL_CONNECTION_FAILED") || message.Contains("ERR_CONNECTION_TIMED_OUT") || message.Contains("ERR_SOCKS_CONNECTION_FAILED") || message.Contains("ERR_SOCKS_CONNECTION_HOST_UNREACHABLE") || message.Contains("ERR_PROXY_CONNECTION_FAILED") || message.Contains("ERR_NAME_RESOLUTION_FAILED") || message.Contains("ERR_MANDATORY_PROXY_CONFIGURATION_FAILED")))
            {
                ...
                } catch (Exception ex)
                {
                    BrowserstackPatcher.BrowserStackLog($"Exception while finalizer: {ex.ToString()}");
                        ...
                        if (__exception != null)
                        {
                            return new BrowserStackException(
                                message,
                                Regex.Replace(__exception.StackTrace, @"(_Patch)(\d+)", "")
                            );
                        }
...
[HarmonyPatch]
class TestExecutionRecorderStartPatch
{
    private static readonly FileLogger logger = new FileLogger();
    public static string frameWork = (Environment.GetEnvironmentVariable("frameWork") == "xunit") ? "XUnit" : "MSTest";
    public static Microsoft.VisualStudio.TestPlatform.ObjectModel.TestCase testContextObj;

    static MethodBase TargetMethod()
    {
        Assembly assembly = typeof(RunContext).Assembly;
        return assembly.GetType("Microsoft.VisualStudio.TestPlatform.CrossPlatEngine.Adapter.TestExecutionRecorder").GetMethod("RecordStart");
    }

    static List<string> GetTestMethodAttributes(string fullName, List<string> attributesList, Assembly assembly)
    {
        List<string> attributes = new List<string>();
        string[] parts = fullName.Split('.');
        foreach (var attribute in attributesList)
        {
            var attributeType = Type.GetType(attribute);
            Type? type = assembly.GetType(string.Join(".", parts.Take(parts.Length - 1)));
            MethodInfo? methodInfo = type?.GetMethod(parts[parts.Count() - 1]);

            if (methodInfo != null && attributeType != null) {
                attributes = methodInfo.GetCustomAttributes(true)
                .Where(el => el.GetType() == attributeType)
                .Select(attr => string.Format("Ignore=\"{0}\"", attributeType.GetProperty("IgnoreMessage")?.GetValue(attr)?.ToString()))
                .ToList();
            }
        }
        return attributes;
    ...
    public static List<string> GetTags(Microsoft.VisualStudio.TestPlatform.ObjectModel.TestCase testCase)
    {
        List<string> tags = new List<string>();
        try
        {
            foreach (var item in testCase.GetProperties())
            {
                if (item.Key.ToString().Equals("MSTestDiscoverer.TestCategory")) {
                    foreach (var category in ((String[]) item.Value))
                    {
                        tags.Add(string.Format("TestCategory=\"{0}\"",category));
                    }
                } else if (item.Key.ToString().Equals("Description")) {
                    tags.Add(string.Format("Description=\"{0}\"", item.Value));
                } else if (item.Key.ToString().Equals("WorkItemIds")) {
                    foreach (var workItemId in ((String[]) item.Value))
                    {
                        tags.Add(string.Format("WorkItemId=\"{0}\"", workItemId));
                    }
                } else if (item.Key.ToString().Equals("TestObject.Traits")) {
                    foreach (var trait in ((KeyValuePair<string, string>[]) item.Value))
                    {
                        tags.Add(string.Format("{0}=\"{1}\"", trait.Key, trait.Value));
                    }
                }
            }
            Assembly assembly = typeof(RunContext).Assembly;
            List<string> additionalAttributes = GetTestMethodAttributes(testCase.FullyQualifiedName, new List<string>() { "Microsoft.VisualStudio.TestTools.UnitTesting.IgnoreAttribute, Microsoft.VisualStudio.TestPlatform.TestFramework" }, BrowserStackSDK.Automation.Context.executingAssembly ?? Assembly.GetCallingAssembly());
            tags.AddRange(additionalAttributes);
        }
        catch (Exception ex)
        {
            BrowserstackPatcher.BrowserStackLog("Exception while extracting tags in test start hook : "+ ex.ToString());
        }

        return tags;
    }

    public static List<string> getSkipReason(Microsoft.VisualStudio.TestPlatform.ObjectModel.TestResult testResult) {
        List<string> skipReasons = new List<string>();

        try {
            foreach (var item in testResult.Messages) {
                if (item.Category.ToString().Equals("StdOutMsgs")) {
                    skipReasons.Add(string.Format("SkipReason=\"{0}\"",item.Text.ToString()));
                }
            }
        } catch(Exception ex) {
            BrowserstackPatcher.BrowserStackLog("Exception while extracting skip reason in test finish hook : "+ ex.ToString());
        }

        return skipReasons;
    }

    static void Prefix(Microsoft.VisualStudio.TestPlatform.ObjectModel.TestCase testCase)
    {
        logger.Info($"Test: {testCase.FullyQualifiedName} with display name: {testCase.DisplayName} has Started");
    
        testContextObj = testCase;
        Test testContext = new Test(testCase.DisplayName, testCase.FullyQualifiedName, testCase.Id.ToString(), frameWork, GetTags(testCase));
        BrowserStackSDK.Automation.Context automationContext =  BrowserStackSDK.Automation.Context.AddOrGet(testCase.FullyQualifiedName, testCase.DisplayName, testContext);
        automationContext.sessionName = testCase.FullyQualifiedName + " " + testCase.DisplayName;
        automationContext.FullyQualifiedName = testCase.FullyQualifiedName;
        automationContext.insideTestMethods = true;

        BrowserStackSDK.TestObservability.EventsHandler.TestEventsHandler.ReceiveTestContext(automationContext, "TestRunStarted");
        BrowserStackSDK.TestObservability.EventsHandler.TestEventsHandler.TestCaseStartedEvent(automationContext);
    }
}

[HarmonyPatch]
class TestExecutionRecorderResultPatch
{
    private static readonly FileLogger logger = new FileLogger();
    static string frameWork = (Environment.GetEnvironmentVariable("frameWork") == "xunit") ? "XUnit" : "MSTest";

    static MethodBase TargetMethod()
    {
        Assembly assembly = typeof(RunContext).Assembly;
        return assembly.GetType("Microsoft.VisualStudio.TestPlatform.CrossPlatEngine.Adapter.TestExecutionRecorder").GetMethod("RecordResult");
    }
    static void Prefix(Microsoft.VisualStudio.TestPlatform.ObjectModel.TestResult testResult)
    {
        logger.Info($"Test: {testResult.TestCase.FullyQualifiedName} with display name: {testResult.TestCase.DisplayName} has Finished");

        List<string> skipReasons = TestExecutionRecorderStartPatch.getSkipReason(testResult);
        BrowserStackSDK.Automation.Context? automationContext = BrowserStackSDK.Automation.Context.AddOrGet(testResult.TestCase.FullyQualifiedName, testResult.TestCase.DisplayName);
        try
        {
            if (automationContext == null || automationContext.TestInfo == null || automationContext.TestInfo.uuid == null)
            {
                Microsoft.VisualStudio.TestPlatform.ObjectModel.TestCase testCase = testResult.TestCase;
                Test testContext = new Test(testCase.DisplayName, testCase.FullyQualifiedName, testCase.Id.ToString(), frameWork, TestExecutionRecorderStartPatch.GetTags(testCase));

                automationContext.TestInfo = testContext;
                automationContext.TestInfo.StartedAt = testResult.StartTime.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ");
                automationContext.sessionName = testCase.FullyQualifiedName + " " + testCase.DisplayName;
                automationContext.FullyQualifiedName = testCase.FullyQualifiedName;
                automationContext.insideTestMethods = true;
        ...
        {
             BrowserStackSDK.TestObservability.Helper.Utils.TestObservabilityUtils.BrowserStackLog("Error in sending start at TestFinish " + ex.ToString());
        }


        try {
            automationContext.TestInfo.StartedAt = testResult.StartTime.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ");
            automationContext.TestInfo.FinishedAt = testResult.EndTime.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ");
            automationContext.TestInfo.FailureReason = testResult.ErrorMessage;
            automationContext.TestInfo.ErrorStacktrace = testResult.ErrorStackTrace;
            automationContext.TestInfo.result = testResult.Outcome.ToString();
            if(skipReasons.Count > 0) {
                automationContext.TestInfo.tags = skipReasons;
            }
        } catch (Exception ex)
        {
            BrowserstackPatcher.BrowserStackLog("Exception while extracting test result in hook : "+ ex.ToString());
        }
        if (automationContext != null) {
            BrowserStackSDK.TestObservability.EventsHandler.TestEventsHandler.ReceiveTestContext(automationContext, "TestRunFinished");
            if (frameWork == "XUnit") {
                BrowserStackSDK.TestObservability.EventsHandler.TestEventsHandler.TestCaseFinishedEvent(automationContext, testResult.TestCase.FullyQualifiedName, testResult.TestCase.DisplayName);
            } else {
                BrowserStackSDK.TestObservability.EventsHandler.TestEventsHandler.TestCaseFinishedEvent(automationContext);
            }
        }
    }
}
