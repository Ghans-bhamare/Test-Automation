...
namespace WK.UITest.CanadaPageModels
{
    public class VideoLibraryPage
    {
        ...
        private By mostViewed = By.XPath("//div[@class='buttons']| //div[contains(text(),'Les plus consultés')]");
        private By hotTopics = By.XPath("//div[contains(text(),'Hot Topics')] | //div[contains(text(),'Sujets d')]");
        private By clearAll = By.XPath("//div[@class='clear-filter-text']");

        private By resultsPerPage = By.XPath("(//div[@aria-label='dropdown trigger'])[2]");
        ...
        public void verifyVideo()
        {
            ...
            try
            {
                int cnt = driver.FindElements(By.XPath("(//div[@class='video-card'])[1]")).Count;
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    if (driver.FindElement(By.XPath("//div[@class='p-dropdown p-component p-inputwrapper']")).Text == "Français")
                    {
                        if (CommonActions.IsElementPresent(driver, By.XPath("(//div[@class='video-card'])[" + (i + 1) + "]//descendant::div[2]//div[4]")))
                        {
                            articleAvailable = "Oui";
                        }
                        else articleAvailable = "Non";
                        ...
                        String[,] data = {
                                        ...
                                        { "Durée : ", duration },
                                        { "Vues : ", views },
                                        { "Article disponible? : ", articleAvailable }
                                        };
